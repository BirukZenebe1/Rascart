    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50">
      <div className="container mx-auto px-4 py-10">
        <div className="mb-6 flex flex-wrap items-center gap-3">
          <h1 className="text-3xl font-black text-slate-900">Seller Chats</h1>
            ...
            {!selectedThread ? (
              ...
              <>
                <div className="border-b border-slate-200 px-5 py-4">
                  ...
                  <div className="text-xs text-slate-500">Buyer: {selectedThread.buyer_username || 'Unknown'}</div>
                </div>
                ...
                <div className="flex-1 overflow-auto px-5 py-4 space-y-3 bg-slate-50">
                  {(selectedThread.messages || []).length === 0 ? (
                    ...
                    (selectedThread.messages || []).map((message, index) => {
                      const isSellerMessage = message.sender_id === selectedThread.seller_id;
                      return (
                        <div key={`${message.created_at}-${index}`} className={`flex ${isSellerMessage ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${
                            ...
                            <div className={`mt-1 text-[11px] ${isSellerMessage ? 'text-cyan-100' : 'text-slate-400'}`}>
